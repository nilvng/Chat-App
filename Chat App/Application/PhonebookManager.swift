...
class PhonebookManager {
    ...
    private var friends : [String: FriendContact] = [:]
...
//        NotificationCenter.default.removeObserver(self)
    ...
    func fetchData(forceReload :Bool = false,_ complilationHandler: @escaping (Result<String,Error>) -> () ){
        ...
        self.friendsQueue.async {
            ...
            for cnContact in cnContacts{
                ...
                let nativeFriend = FriendContact(contact: cnContact)
    ...
    func add(_ contact: FriendContact ){
        self.friendsQueue.async {
            // save copy in memo
    ...
    func delete(_ contact: FriendContact){
        self.friendsQueue.async {
    ...
    func update(_ contact: FriendContact){
        self.friendsQueue.async {
    ...
    func getContactList()->[String: FriendContact]{
        let immutableList = self.friends // Copying this variable into a new variable for immutability
        return immutableList
    }
    
    func getContact(key: String) -> FriendContact? {
        self.friendsQueue.sync {
        return self.friends[key]
        }
    }
    
    func listToDict(listFriend: [FriendContact]) -> [String:FriendContact]{
        var dict : [String: FriendContact] = [:]
        for i in listFriend{
            dict[i.uid] = i
        }
        return dict
...
extension PhonebookManager {
    ...
    private func addFriend(_ person: FriendContact){
        friends[person.uid] = person
    }
    
    private func deleteFriend(_ person: FriendContact) {
        // remove in-memo
        friends.removeValue(forKey: person.uid)
    }
    
    private func updateFriend(_ person: FriendContact){
        friends[person.uid] = person
    }
    func contains(_ person:FriendContact) -> Bool{
        return friends[person.uid] != nil
    }
    func get(key: String) -> FriendContact?{
        return friends[key]
    }
    func getAll() -> [String: FriendContact]{
        return friends
    }
}
...
extension PhonebookManager {
    ...
    private func saveContactToNative(_ contact: FriendContact, completition: @escaping (Bool) -> Void) {
        guard isAuthorized else {
            completition(false)
            return
        }
    ...
    private func removeNativeContact(_ contact: FriendContact, completition: @escaping (Bool) -> Void) {
        guard isAuthorized else {
            completition(false)
            return
        }
    ...
    private func updateNativeContact(_ contact: FriendContact, completition: @escaping (Bool) -> Void) {
        guard isAuthorized else {
            completition(false)
            return
        }
