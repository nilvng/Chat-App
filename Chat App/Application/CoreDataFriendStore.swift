...
class CoreDataFriendStore : FriendStore{
    ...
    // MARK: APIs
    func loadData(completion: @escaping (Result<[FriendContact], Error>) -> Void) {
        let  context = getContext()
        let request : NSFetchRequest<FriendCoreData> = FriendCoreData.fetchRequest()
        context.performAndWait {
            do {
                let allFriends = try context.fetch(request)
                let universalTypeFriends : [FriendContact] = allFriends.compactMap {f in
    ...
    func toFriendCoreData(_ person: FriendContact, completion: @escaping (FriendCoreData) -> Void){
        let context =  getContext()
        context.perform {
    ...
    func toFriend(_ person: FriendCoreData) -> FriendContact {
        var friend = FriendContact()
        friend.uid = person.uid!
        friend.firstName = person.firstName!
        friend.lastName = person.lastName!
    ...
    func toFriendList(_ friends: [FriendCoreData]) -> [FriendContact]{
